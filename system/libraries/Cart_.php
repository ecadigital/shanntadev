...
class CI_Cart {
	...
	var $product_name_rules	= '\.\:\-_ a-z0-9ก-๙ \เ'; // alpha-numeric, dashes, underscores, colons or periods, thai lang
	...
	function _update($items = array())
	{
		...
		if ( ! isset($items['qty']) OR ! isset($items['rowid']) OR ! isset($items['options']) OR ! isset($this->_cart_contents[$items['rowid']]))
		{
			return FALSE;
		}
		...
		if ($this->_cart_contents[$items['rowid']]['qty'] == $items['qty'] && $items['qty'] != 0)
		{
			return FALSE;
		}
		...
		if(!empty($items['options'])){
			foreach($items['options'] as $options){
				$this->_cart_contents[$items['rowid']]['options']['team'] = $items['options']['team'];
			}
		}
